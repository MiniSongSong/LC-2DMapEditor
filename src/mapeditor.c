...
static void load_res(void)
{
	...
	/* 从文件中载入图像数据 */
	Graph_LoadImage( IMG_PATH_BTN_HORIZ, &img_btn_horizflip );
	Graph_LoadImage( IMG_PATH_BTN_VERTI, &img_btn_vertiflip );
	Graph_LoadImage( IMG_PATH_ICON, &wnd_icon );
	Graph_LoadImage( IMG_PATH_BTN_RESIZE, &img_btn_resize );
	Graph_LoadImage( IMG_PATH_BTN_SAVE, &img_btn_save );
	Graph_LoadImage( IME_PATH_BTN_DEL, &img_btn_del );
	/* 载入地图图块资源 */
	i = Graph_LoadImage( IMG_PATH_MAP, &map_res );
	if( i != 0 ) {
	...
	/* 载入地图对象资源 */
	i = Graph_LoadImage( IMG_PATH_OBJS, &obj_res );
	if( i != 0 ) {
		LCUI_MessageBoxW( MB_ICON_ERROR, 
	...
	for(i=0; i<MAP_OBJ_TOTAL; ++i) {
		...
		//map_objs[i].color_type = COLOR_TYPE_RGBA;
	}
}
